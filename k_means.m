function [center_U, Mk ] = k_means(center_number, training_sample_DS, number_of_sampleclass)
...
for i = 1: number_of_sampleclass
  ...
  %  Mk(i)
    ...
    if(Mk(i) == 1)
            ...
            % if one of the dataset's center number larger than its samples
            % number here will occur a error
            temp_Clustering = kmeans(tempDS{i}',Mk(i),'emptyaction', 'singleton' ,'start' , tempDS{i}(:,1:Mk(i))' ); 
            for j=1:Mk(i)
                    center_U = [center_U mean( tempDS{i}(:,find(temp_Clustering == j)),2)] ;
            end
    end
end
